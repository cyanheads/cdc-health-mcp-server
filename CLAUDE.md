...
| `cdc_query_dataset` | tool | Execute SoQL queries — filter, aggregate, sort, full-text search. Continuation is measured by an over-fetch probe, not the row count, and the page is bounded by a character budget over the whole serialized result (each row charged as JSON and as its table line); discloses `truncated`/`shown`/`cap`/`nextOffset`, never a `totalCount`. An empty page at `offset > 0` spends one probe at offset 0 to tell an offset past the end from a no-match. Fails `not_queryable` when every row comes back with no fields and the asset has no columns. |
| `cdc_query_wonder` | tool | Query CDC WONDER for national deaths, population, crude/age-adjusted rates. Grouped by year/age/sex/race, filtered by ICD-10 cause. A `database` enum selects one of five mortality databases — D76 (default), D176 provisional, D158, D77, D157. Every page is bounded by the same 200,000-character whole-result budget as `cdc_query_dataset` (each row charged as JSON and as its table line, plus its cell notes; caveats, messages and the query echo reserved at their measured size), so an omitted `limit` takes as many rows as fit. `limit`/`offset` page the parsed rows with exact `totalCount`/`truncated`/`nextOffset`, re-basing `cellNotes` onto the page while `caveats`/`messages` stay whole. |
...
- Every relational input check — a reversed `year_range`, a repeated `group_by` dimension, per-database year spans, the `mcd_icd10`-needs-a-multiple-cause-database rule, the `999--999` marker off `provisional` — is enforced **in the handler** before any request, never a Zod `refine`: a refinement adds nothing to the emitted JSON Schema and fails argument parsing as `invalid_arguments`, out of reach of the declared `invalid_query` recovery. Same pattern #27 established for `cdc_discover_datasets`. Union inputs (`age_groups`, both cause lists) are deduplicated in one normalization step that runs before `measuresFor`, the request builder, and `effectiveQuery` read them — a repeated age group otherwise reads as two and keeps an age-adjusted rate one group cannot support.
- `cause_icd10` / `mcd_icd10` take `string | string[]` (up to 50). Each entry is one `F_<ID>.V2` / `.V13` `<value>` — WONDER ORs values within a finder and ANDs the two finders — and `I_` carries them space-joined. WONDER accepts only nodes of its ICD-10 tree (chapter, block, code): `X40-X49` works, `X40-X44` is rejected as an invalid code, so a code set goes in as a list.
- Send bare headers — a browser-looking User-Agent/Origin trips an upstream Akamai 403.
- Hard 16-second gap between requests, enforced by the service (429 otherwise). It is measured from the end of the previous response, not from when its request was issued — WONDER rejects a request sent 15 s after the previous one started. The stamp lives in `send()`'s `finally`, so the network-error, 429, and malformed-body paths space the next call too. The limit is per source IP and shared across databases: one process-wide gap, never a per-database limiter. Overlapping calls queue in one process-wide lane — `query()` chains each call on the previous call's settlement (never its success) before it measures the gap, so concurrent calls go out one at a time instead of reading the same stamp and firing together. A call aborted while queued rejects at once and sends nothing; its lane slot still waits for the call ahead, and only a sent request stamps.
...
- Caveat, footnote, and message anchors become Markdown links (`toLinkedText` in `xml-parser.ts`). Anchors are converted on the raw markup before any tag comes off, each segment is decoded once, and the assembled string is never decoded again. Hrefs resolve with `URL.parse(href, 'https://wonder.cdc.gov/')` and only an `http:`/`https:` result becomes a link — `URL` passes `javascript:` through — while a fragment-only or unparseable href keeps its text alone. `toPlainText` stays link-free for Socrata.
...
Directory-based, grouped by minor series via the `.x` semver-wildcard convention. Source of truth: `changelog/<major.minor>.x/<version>.md` (e.g. `changelog/0.8.x/0.8.6.md`) — one file per release, shipped in the npm package. At release, author the per-version file with a concrete version and date, then run `bun run changelog:build` to regenerate the rollup. `changelog/template.md` is a **pristine format reference** — never edited or moved; read it for the frontmatter + section layout when scaffolding. `CHANGELOG.md` is a **navigation index** (header + link + summary per version), regenerated by `bun run changelog:build` — devcheck hard-fails on drift; never hand-edit it.
