...
Thanks for using `cdc-health-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

Open one from the **Issues** tab and pick the [**Bug Report**](https://github.com/cyanheads/cdc-health-mcp-server/issues/new?template=bug_report.yml) or [**Feature Request**](https://github.com/cyanheads/cdc-health-mcp-server/issues/new?template=feature_request.yml) form. Both are structured, and filling in the fields is what makes an issue actionable.

PRs welcome; open an issue first for anything larger than a typo.

## Server bug or framework bug?

`cdc-health-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
...
Upstream data problems are a third case: a missing column, a dataset ID that no longer resolves, or a suppressed mortality figure usually reflects what [data.cdc.gov](https://data.cdc.gov) or [CDC WONDER](https://wonder.cdc.gov) returned. Include the dataset ID and the tool arguments so the raw response can be checked.
