...
| `limit` | number | No | Rows to return from the parsed table, 1–5000. Omit for the whole table. |
| `offset` | number | No | Index of the first row to return, 0–10,000. Defaults to 0. |

**Returns:** `{ rows, rowCount, database, databaseTitle, caveats, cellNotes, messages, suppressedCount }`. Each row carries the requested `group_by` dimensions followed by `deaths`, `population`, `crude_rate`, and — when age standardization is possible — `age_adjusted_rate`, all rates per 100,000. `database` is the selected database's dataset code (`D76`, `D176`, …) and `databaseTitle` CDC's own title for it, so a result names its own source rather than needing one looked up. `caveats` holds CDC's own footnotes for the result set. `effectiveQuery` echoes the database, grouping, and filters as a readable summary, and `totalCount` the rows in the whole table; a bounded page adds `truncated`/`shown`/`cap` and a `nextOffset`.

#### Row pagination

A four-dimension grouping is a legitimate epidemiological query and a large response: 22 years × 12 age groups × 2 sexes × 4 bridged race groups is ~2,100 combinations on `D76` before CDC hides the zero-death and suppressed-death rows, and a `["year","age_group","sex","race"]` run on `D158` was reported at 933 rows / ~137 KB of `structuredContent`. `limit` and `offset` bound that, keeping the field names and semantics `cdc_query_dataset` and `cdc_get_dataset_schema` use.
...
**Decision: omitting `limit` returns the whole table.** The tool has always returned it, callers hold that behaviour, and the paging exists to give a caller a way *down* from a large table rather than to impose one. `offset` defaults to 0 and an offset at or past the row total returns an empty page with a notice, not an `invalid_query` — a spent offset is a finished walk, not a malformed request, and reporting it as an error would be indistinguishable from a query that matched nothing.
